...
function getMicState {
    micState=$(pacmd list-sources | grep -A15 "index: $active_source$" | grep 'muted:' | awk -F: '{print $2}' | tr -d ' ')
}
        ...
        while true; do
            {
                ...
                then
                    ...
                    read -r event || break
                    if ! echo "$event" | grep -e "on card" -e "on sink" -e "on source"
                    then
                        # Avoid double events
                        continue
                    fi
...
function output() {
    ...
    local vol_state='Unknown'
    local mic_state='Unknown'

    if [ "${curStatus}" = 'yes' ]
    then
        vol_state='Mute'
    else
        vol_state="Vol $curVol%"
    fi
